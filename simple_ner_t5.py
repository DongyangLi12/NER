...
Original file is located at
    https://colab.research.google.com/drive/1u3xSdyF-v_8bUWhywBYaOpdtlFQEbLsh
"""
...
def get_class_prediction(prompt, tokeniser, model, device):
    ...
    decoded = tokeniser.batch_decode(output,
                                     skip_special_tokens=True
                                     )
    # take the first word
    labels = []
    for txt in decoded:
        first_word = txt.strip().split() or "O"
        labels.append(first_word[0] if first_word else "O")
...
for epoch_n in tqdm(range(n_epochs)):
    shuffle(train_set)
    epoch_loss = train_epoch(train_set_simple, batch_size,
                             tokeniser, model, device, optim)
    print(f'Epoch {epoch_n+1} loss:', round(epoch_loss, 2))

    metrics = validate_epoch(
        dev_set_simple, tokeniser, model, device)
...
test_on_split(test_set_simple, "In-Domain Test")
test_on_split(test_set_ood_simple,  "Out-of-Domain Test")